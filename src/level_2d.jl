"""Convert 3D into 2D before running qhull"""
function hull2d(points::Vector{Point3{T}}) where {T}
    span = compute_span(points)
...
struct Level2D{T}
    ...
    function Level2D{T}(s::Subhull{T}) where {T}
        verts = collect(vertices(s))
        ps = points(s, verts)
        in_order = verts[hull2d(ps)]
        # println("Level2D ", s.points, " ", in_order)
        new(s, in_order)
    end
end
