@testset "persistent_rb" begin
    ...
    @testset "persistent_rb_insert_root" begin
        ...
        @test tour_rb(root1, 1.0, true) == [5]
        root2 = insert_root(root1, 2.0, 3)
        @test tour_rb(root2, 2.0, true) == [3, 5]
        root3 = insert_root(root2, 3.0, 1)
        @test tour_rb(root3, 3.0, true) == [1, 3, 5]
        root4 = insert_root(root3, 4.0, 2)
        @test tour_rb(root4, 4.0, true) == [1, 2, 3, 5]
        root5 = insert_root(root4, 5.0, 4)
        @test tour_rb(root5, 5.0, true) == [1, 2, 3, 4, 5]
        @test tour_rb(root2, 2.0) == [3, 5]
        @test tour_rb(root4, 4.0) == [1, 2, 3, 5]
    end
    ...
    @testset "persistent_rb_delete_root" begin
        ...
        @test tour_rb(root2, 6.0, true) == [1, 2, 3, 5]
        root3 = delete_root(root2, 7.0, 1)
        root4 = delete_root(root3, 8.0, 5)
        @test tour_rb(root4, 8.0, true) == [2, 3]
        @test tour_rb(root, 5.0) == [1, 2, 3, 4, 5]
        @test tour_rb(root2, 6.0) == [1, 2, 3, 5]
        @test tour_rb(root3, 7.0) == [2, 3, 5]
    end
end